...
export default function EditFounderSection() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/founder/${id}`)
        if (res.ok) {
          ...
          setForm({
            ...
            cardDescription: data.cardDescription?.length ? data.cardDescription : [''],
        ...
        console.error('Error loading founder section:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 lg:space-y-6">
      <div>
        ...
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#f5f3ee]">Edit Founder Section</h1>
      </div>
      ...
      <form onSubmit={handleSave} className="glass-effect rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-[#c8a75e]/20 space-y-4 sm:space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={form.slug} onChange={e => setForm({...form, slug: e.target.value})} className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-[#0b0f2a]/40 border border-[#c8a75e]/20 rounded-lg sm:rounded-xl text-sm text-[#f5f3ee] focus:outline-none focus:border-[#c8a75e]/50" required />
          </div>
          <div>
            <label className="block text-xs font-medium text-premium-light mb-1">Badge Label</label>
            <input type="text" value={form.badgeLabel} onChange={e => setForm({...form, badgeLabel: e.target.value})} className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-[#0b0f2a]/40 border border-[#c8a75e]/20 rounded-lg sm:rounded-xl text-sm text-[#f5f3ee] focus:outline-none focus:border-[#c8a75e]/50" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-premium-light mb-1">Card Title (role description)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-premium-light mb-1">Card Subtitle (person name)</label>
        ...
        <div className="flex gap-2 pt-2">
          <button type="submit" disabled={saving} className="flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-2 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg sm:rounded-xl transition-colors text-xs sm:text-sm font-medium">
            <Save className="w-3 h-3 sm:w-4 sm:h-4" /> {saving ? 'Saving...' : 'Save'}
          </button>
